...
@interface UWDatePickerView ()<UIPickerViewDataSource,UIPickerViewDelegate>
{
    NSString *timesText;
    NSString *pickTime; //pickView选择的时间
    NSString *dateTime; //datePick选择的时间
}
...
- (void)awakeFromNib
{
    ...
    self.sureBtn.layer.borderColor = NavColor.CGColor;
    ...
    _timeData = @[@"00",@"30"];
    pickTime = [_timeData firstObject];
}
...
- (IBAction)sureBtnClick:(id)sender {
    ...
    NSString *clipsTime = [self.selectDate substringToIndex:13];
    NSString *times = [NSString stringWithFormat:@"%@:%@",clipsTime,pickTime];
    [self.delegate getSelectDate:times type:self.type];
...
{
    return _timeData[row];
}

-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    NSLog(@"row = %@",_timeData[row]);
    pickTime = _timeData[row];
}
